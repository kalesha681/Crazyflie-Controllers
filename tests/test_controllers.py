...
def sample_inputs():
    ...
    cur_ang_vel = np.array([0.0, 0.0, 0.0])
...
class TestControllers:
    ...
    def test_compute_control_interface(self, ControllerClass, sample_inputs):
        ...
        assert output.shape == (
            4,
        ), f"Output shape mismatch: expected (4,), got {output.shape}"
    ...
    def test_determinism(self, ControllerClass, sample_inputs):
        ...
        np.testing.assert_array_equal(
            out1, out2, err_msg="Controller is non-deterministic"
        )
    ...
    def test_stability_bounded_output(self, ControllerClass, sample_inputs):
        ...
        output = ctrl.compute_control(**sample_inputs)
        assert np.all(
            np.abs(output) < HARD_LIMIT
        ), f"Control output exceeded hard limit {HARD_LIMIT}"
