...
def run_simulation(config: ExperimentConfig, ctrl_name: str):
    ...
    env = CtrlAviary(
        drone_model=DroneModel.CF2X, num_drones=1, physics=Physics.PYB, gui=config.gui
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Crazyflie Controllers Experiment Pipeline"
    )

    # Mandatory
    parser.add_argument(
        "--trajectory", type=str, required=True, choices=TRAJECTORY_REGISTRY.keys()
    )
    parser.add_argument(
        "--controllers",
        type=str,
    ...
    parser.add_argument(
        "--gui",
        action="store_true",
        help="Enable PyBullet GUI (WARNING: May crash on headless)",
    )
    parser.add_argument(
        "--save",
        action="store_true",
        default=True,
        help="Save CSV data (Default: True)",
    )
    parser.add_argument(
        "--no-save", action="store_false", dest="save", help="Disable CSV saving"
    )
    parser.add_argument(
        "--plot",
        action="store_true",
