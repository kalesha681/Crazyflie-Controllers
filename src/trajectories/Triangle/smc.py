...
def run():
    ...
    plt.plot(ref[:,0], ref[:,1], 'k--', label='Target Triangle')
    plt.plot(act[:,0], act[:,1], 'g', label='Drone (SMC)')
    plt.legend(); plt.grid(True); plt.axis('equal'); plt.title("Triangle Tracking (SMC)")
    filename = "outputs/plots/triangle/smc.png"
