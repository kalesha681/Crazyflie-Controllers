...
class Logger:
    ...
    def log(
        ...
        assert target_pos.shape == (
            3,
        ), f"target_pos shape mismatch: {target_pos.shape}"
        assert control_rpm.shape == (
            4,
        ), f"control_rpm shape mismatch: {control_rpm.shape}"

        # 2. NaN Checks (Fail Fast)
        if not np.all(np.isfinite(pos)):
            raise ValueError("pos contains NaN/Inf")
        if not np.all(np.isfinite(vel)):
            raise ValueError("vel contains NaN/Inf")
        if not np.all(np.isfinite(rpy)):
            raise ValueError("rpy contains NaN/Inf")
        if not np.all(np.isfinite(target_pos)):
            raise ValueError("target_pos contains NaN/Inf")
        if not np.all(np.isfinite(control_rpm)):
            raise ValueError("control_rpm contains NaN/Inf")
        ...
        entry = {
            ...
            "pitch": float(rpy[1]),
            "yaw": float(rpy[2]),
            "x_ref": float(target_pos[0]),
            "y_ref": float(target_pos[1]),
            "z_ref": float(target_pos[2]),
            "ex": float(error[0]),
            "ey": float(error[1]),
            "ez": float(error[2]),
        ...
        if len(entry) != len(self.COLUMNS):
            raise ValueError(
                f"Log entry length mismatch. Expected {len(self.COLUMNS)}, got {len(entry)}"
            )

        self.data_buffer.append(entry)

    def save_as_csv(
        self, filename: str, metadata: Optional[Dict[str, str]] = None
    ) -> None:
        """
        ...
        if df.isnull().values.any():
            raise ValueError(
                "NaN values detected in log buffer. Logs must be complete."
            )
